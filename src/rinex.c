...
*         International GNSS Service (IGS), RINEX Working Group and Radio
*         Technical Commission for Maritime Services Special Committee 104
*         (RTCM-SC104), December 1, 2020
*     [11] RINEX The Receiver Independent Exchange Format Version 4.00,
*         International GNSS Service (IGS), RINEX Working Group and Radio
*         Technical Commission for Maritime Services Special Committee 104
*         (RTCM-SC104), December 1, 2021
*     [12] RINEX The Receiver Independent Exchange Format Version 4.01,
*         International GNSS Service (IGS), RINEX Working Group and Radio
*         Technical Commission for Maritime Services Special Committee 104
*         (RTCM-SC104), July 10, 2023
*     [13] RINEX The Receiver Independent Exchange Format Version 4.02,
*         International GNSS Service (IGS), RINEX Working Group and Radio
*         Technical Commission for Maritime Services Special Committee 104
*         (RTCM-SC104), October 1, 2024
*
* version : $Revision:$
...
static const navtype_t
	navtype_gps[]={NAV_GPS_LNAV,NAV_GPS_CNAV,NAV_GPS_CNV2,NAV_NONE},
	navtype_gal[]={NAV_GAL_INAV,NAV_GAL_FNAV,NAV_NONE},
	navtype_glo[]={NAV_GLO_FDMA,NAV_GLO_L1OC,NAV_GLO_L3OC,NAV_NONE},
	navtype_qzs[]={NAV_QZS_LNAV,NAV_QZS_CNAV,NAV_QZS_CNV2,NAV_NONE},
	navtype_cmp[]={NAV_CMP_D1,NAV_CMP_D2,NAV_CMP_CNV1,NAV_CMP_CNV2,NAV_CMP_CNV3,NAV_NONE},
	navtype_sbs[]={NAV_SBS_L1NV,NAV_NONE},
	navtype_irn[]={NAV_IRN_LNAV,NAV_IRN_L1NV,NAV_NONE};
                        ...
                        char tobs[][MAXOBSTYPE][4], nav_t *nav, sta_t *sta)
{
    ...
    if      (strstr(label,"MARKER NAME"         )) {
        ...
        *tobs[0][nt]='\0';
        ...
        if (nav) { /* dt_ls, dt_lsf, wn_lsf, dn, tsys id */
            int wn_lsf,dn;
            nav->sto[TSYS_GPS].dt_ls=str2num(buff, 0,6);
            nav->sto[TSYS_GPS].dt_lsf=str2num(buff, 6,6);
            wn_lsf=str2num(buff,12,6);
            dn=str2num(buff,18,6);
            nav->sto[TSYS_GPS].tlsf=gpst2time(wn_lsf,dn*86400.0);
        }
    }
...
static void decode_navh(char *buff, nav_t *nav)
{
    ...
    if      (strstr(label,"ION ALPHA"           )) { /* opt ver.2 */
        if (nav) {
            for (i=0,j=2;i<4;i++,j+=12) nav->ion[ION_GPS_LNAV_KLOB].d[i]=str2num(buff,j,12);
        }
    }
    else if (strstr(label,"ION BETA"            )) { /* opt ver.2 */
        if (nav) {
            for (i=0,j=2;i<4;i++,j+=12) nav->ion[ION_GPS_LNAV_KLOB].d[i+4]=str2num(buff,j,12);
        }
    }
    else if (strstr(label,"DELTA-UTC: A0,A1,T,W")) { /* opt ver.2 */
        if (nav) {
            for (i=0,j=3;i<2;i++,j+=19) nav->sto[TSYS_GPS].a[i]=str2num(buff,j,19);
            t=str2num(buff,j,9);j+=9;
            w=str2num(buff,j,9);
            nav->sto[TSYS_GPS].t0=gpst2time(t,w);
        }
    }
    else if (strstr(label,"IONOSPHERIC CORR"    )) { /* opt ver.3 */
        if (nav) {
            if (!strncmp(buff,"GPSA",4)) {
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_GPS_LNAV_KLOB].d[i]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"GPSB",4)) {
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_GPS_LNAV_KLOB].d[i+4]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"GAL",3)) {
                for (i=0,j=5;i<3;i++,j+=12) nav->ion[ION_GAL_IFNV_NEQN].d[i]=str2num(buff,j,12);
                nav->ion[ION_GAL_IFNV_NEQN].idf[0]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"QZSA",4)) { /* v.3.02 */
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_QZS_LNAV_KLOB].d[i]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"QZSB",4)) { /* v.3.02 */
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_QZS_LNAV_KLOB].d[i+4]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"BDSA",4)) { /* v.3.02 */
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_CMP_D1D2_KLOB].d[i]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"BDSB",4)) { /* v.3.02 */
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_CMP_D1D2_KLOB].d[i+4]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"IRNA",4)) { /* v.3.03 */
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_IRN_L1NV_KLOB].d[i]=str2num(buff,j,12);
            }
            else if (!strncmp(buff,"IRNB",4)) { /* v.3.03 */
                for (i=0,j=5;i<4;i++,j+=12) nav->ion[ION_IRN_L1NV_KLOB].d[i+4]=str2num(buff,j,12);
            }
        }
    }
        ...
        if (nav) {
            if (!strncmp(buff,"GPUT",4)) {
                nav->sto[TSYS_GPS].a[0]=str2num(buff, 5,17);
                nav->sto[TSYS_GPS].a[1]=str2num(buff,22,16);
                t=str2num(buff,38, 7);
                w=str2num(buff,45, 5);
                nav->sto[TSYS_GPS].t0=gpst2time(t,w);
            }
            else if (!strncmp(buff,"GLUT",4)) {
                nav->sto[TSYS_GLO].a[0]=-str2num(buff,5,17); /* tau_C */
            }
            else if (!strncmp(buff,"GLGP",4)) {
                nav->sto[TSYS_GLO].a[1]=-str2num(buff, 5,17); /* tau_GPS */
            }
            else if (!strncmp(buff,"GAUT",4)) { /* v.3.02 */
                nav->sto[TSYS_GAL].a[0]=str2num(buff, 5,17);
                nav->sto[TSYS_GAL].a[1]=str2num(buff,22,16);
                t=str2num(buff,38, 7);
                w=str2num(buff,45, 5);
                nav->sto[TSYS_GAL].t0=gpst2time(t,w);
            }
            else if (!strncmp(buff,"QZUT",4)) { /* v.3.02 */
                nav->sto[TSYS_QZS].a[0]=str2num(buff, 5,17);
                nav->sto[TSYS_QZS].a[1]=str2num(buff,22,16);
                t=str2num(buff,38, 7);
                w=str2num(buff,45, 5);
                nav->sto[TSYS_QZS].t0=gpst2time(t,w);
            }
            else if (!strncmp(buff,"BDUT",4)) { /* v.3.02 */
                nav->sto[TSYS_CMP].a[0]=str2num(buff, 5,17);
                nav->sto[TSYS_CMP].a[1]=str2num(buff,22,16);
                t=str2num(buff,38, 7);
                w=str2num(buff,45, 5);
                nav->sto[TSYS_CMP].t0=gpst2time(t,w);
            }
            else if (!strncmp(buff,"SBUT",4)) { /* v.3.02 */
                nav->sto[TSYS_SBS].a[0]=str2num(buff, 5,17);
                nav->sto[TSYS_SBS].a[1]=str2num(buff,22,16);
                t=str2num(buff,38, 7);
                w=str2num(buff,45, 5);
                nav->sto[TSYS_SBS].t0=gpst2time(t,w);
            }
            else if (!strncmp(buff,"IRUT",4)) { /* v.3.03 */
                nav->sto[TSYS_IRN].a[0]=str2num(buff, 5,17);
                nav->sto[TSYS_IRN].a[1]=str2num(buff,22,16);
                t=str2num(buff,38, 7);
                w=str2num(buff,45, 5);
                nav->sto[TSYS_IRN].t0=gpst2time(t,w);
                nav->sto[TSYS_IRN].a[2]=0.0; /* A2 */
            }
        }
    }
    else if (strstr(label,"LEAP SECONDS"        )) { /* opt */
        if (nav) {
            ...
            w=str2num(buff,12,6);
            dn=str2num(buff,18,6);
            nav->sto[TSYS_GPS].tlsf=gpst2time(w,dn*86400.0);
        }
    }
}
/* decode GNAV header --------------------------------------------------------*/
...
static int decode_eph(double ver, int sat, gtime_t toc, const double *data,
					  eph_t *eph, navtype_t navtype)
{  
    eph_t eph0={0};
	int sys,wn_op,idx0,prn;
	double t_tm;
	...
	*eph=eph0;
    ...
    if (sys==SYS_GPS||sys==SYS_QZS) {
        if (ver<4.00||navtype==NAV_GPS_LNAV||navtype==NAV_QZS_LNAV) { /* GPS ver.2,3 or ver.4 with LNAV */
            /* LNAV */
        ...
        } else { /* GPS ver.4 with navid>=1 */
            /* CNAV/CNAV2 */
            time2gpst(toc,&eph->week); /* GPS week */
            eph->Adot=data[3];      /* Adot [m/s] */
            eph->ndot=data[20];     /* n-dot [rad/s^2] */
            eph->urai[0]=(int)data[21]; /* URAI NED0 */
            eph->urai[1]=(int)data[22]; /* URAI NED1 */
            eph->urai[2]=(int)data[26]; /* URAI NED2 */
            eph->urai[3]=(int)data[23]; /* URAI ED */
            eph->tgd[1]=data[27];      /* ISC L1CA [s] */
            eph->tgd[2]=data[28];      /* ISC L2C [s] */
            eph->tgd[3]=data[29];      /* ISC L5I5 [s] */
            eph->tgd[4]=data[30];      /* ISC L5Q5 [s] */

			if (navtype==NAV_GPS_CNAV||navtype==NAV_QZS_CNAV) {
				idx0=30;
            }
            else {
                idx0=34;
                eph->tgd[5]=data[31];  /* ISC L1Cd [s] */
				eph->tgd[6]=data[32];  /* ISC L1Cp [s] */
            }
        ...
        eph->navtype=navtype; /* navid indicates signal type */
		if (ver<4.00||navtype==NAV_CMP_D1||navtype==NAV_CMP_D2) {
            ...
            /* CNAV1/2/3 */
            time2gpst(toc,&eph->week); /* BDT week */
            eph->Adot=data[3];      /* Adot [m/s] */
            eph->ndot=data[20];     /* n-dot [rad/s^2] */
            eph->flag=(int)data[21];      /* sattype 1:GEO,2:IGSO,3:MEO */
            eph->top=bdt2gpst(bdt2time(eph->week,data[22])); /* BDT -> GPST */
            eph->urai[0]=(int)data[23]; /* SISAI oe */
            eph->urai[1]=(int)data[24]; /* SISAI ocb */
            eph->urai[2]=(int)data[25]; /* SISAI oc1 */
            eph->urai[3]=(int)data[26]; /* SISAI oc2 */

            if (navtype==NAV_CMP_CNV1||navtype==NAV_CMP_CNV2) { /* CNAV-1/CNAV-2 */
                idx0=31;
                eph->tgd[0]=data[27];      /* ISC B1Cd [s] (CNAV-1) */
                eph->tgd[1]=data[28];      /* ISC B2ad [s] (CNAV-2) */
                eph->tgd[2]=data[29];      /* TGD B1Cp [s] */
                eph->tgd[3]=data[30];      /* TGD B2ap [s] */
                eph->iodc=(int)data[34]; /* IODC */
                eph->iode=(int)data[38]; /* IODE */
            }
            else { /* CNAV-3 */
                idx0=27;
                eph->tgd[4]=data[idx0+3];   /* TGD B2bI [s] */
            }

            eph->urai[4]=(int)data[idx0]; /* SISMAI */
            eph->svh =(int)data[idx0+1];   /* satH1 */
            eph->flag=(int)data[idx0+2]; /* satType 1:GEO,2:IGSO,3:MEO */
            eph->ttr=bdt2gpst(bdt2time(eph->week,data[idx0+4])); /* BDT -> GPST */
            eph->ttr=adjweek(eph->ttr,toc);
        }
		...
		if (ver<3.03) {
			trace(3,"rinex version error: IRNSS is not supported in ver=%f\n",ver);
            return 0;
		}
		else if ((ver>=3.03&&ver<4.00)||navtype==NAV_IRN_LNAV) {
            /* IRNSS LNAV */
            ...
            /* IRNSS L1NAV */
            time2gpst(toc,&eph->week); /* IRNSS week */
            eph->Adot=data[3];      /* Adot [m/s] */
            eph->ndot=data[20];     /* n-dot [rad/s^2] */

            eph->flag=(int)data[22];      /* reference signal flag (RSF) */
            eph->sva=uraindex(data[23]);  /* URA index (m->index) */
            eph->svh =(int)data[24];      /* SV health */

            eph->tgd[0]=data[25];      /* TGD L1P-L5 (RSF=0) [s] */
            eph->tgd[1]=data[26];      /* TGD S-L5 (RSF=1) [s] */
            eph->tgd[2]=data[27];      /* ISC S(L1P) (RSF=0) [s] */
            eph->tgd[3]=data[28];      /* ISC L1D(L1P) (RSF=0) [s] */
            eph->tgd[4]=data[29];      /* ISC L1P(S) (RSF=1) [s] */
            eph->tgd[5]=data[30];      /* ISC L1D(S) (RSF=1) [s] */
                       ...
                       geph_t *geph, navtype_t navtype)
{
    ...
    *geph=geph0;
    ...
    geph->taun=-data[0];       /* -taun SV clock bias [s] */
    geph->gamn= data[1];       /* +gamman SC clock drift [s/s] */
    ...
    geph->svh=(int)data[ 6];
	if (ver<4.00||(ver>=4.00&&navtype==NAV_GLO_FDMA)) { /* FDMA */
        geph->frq=(int)data[10];
#if 0 /*  output dtaun instead of age */
        geph->dtaun=data[14];
#else
        geph->age=(int)data[14]; /* age of operation information [days] */
#endif
		geph->flag=(int)data[15]; /* b8-7:M,b6:P4,b5:P3,b4:P2,b3-2:P1,b1-0:P */
		geph->dtaun=data[16];
        ...
        /* data[10]: data validity */
        geph->beta=data[2]; /* SV clock drift rate [s/s^2] */
		geph->tgd[0]=data[14]; /* TGD L2OCp */

        /* Line 4 */
		geph->flag=((int)data[15])<<7;  /* M */
		geph->src=(int)data[16];
		geph->aode=(int)data[17];
		geph->aodc=(int)data[18];
        /* Line 5 */
		geph->flag|=((int)data[19])<<4; /* P2 */
		geph->tin=data[20];
		geph->tau1=data[21];
		geph->tau2=data[22];
		/* Line 6 */
		...
		geph->dpos[2]=data[30];
		/* Line 8 */
		geph->urai[0]=(int)data[31];
		geph->urai[2]=(int)data[32];
		geph->tof=gpst2time(week,data[34]);
	}
...
/* select navigation IDs */
static navtype_t select_navtype(int sys, const char *id, int *sz)
{
    int j,*sz_=NULL;
	char **navids=NULL;
	int sz_t[7][5]={{31,35,39,0,0},{31,31,0,0,0},{19,35,35,0,0},
			{31,35,39,0,0},{31,31,39,39,35},{15,0,0,0,0},{31,35,0,0,0}};
	navtype_t navtype=NAV_NONE;
	const navtype_t *p;

	const navtype_t nt_gps[3] = {NAV_GPS_LNAV,NAV_GPS_CNAV,NAV_GPS_CNV2};
	const navtype_t nt_gal[2] = {NAV_GAL_INAV,NAV_GAL_FNAV};
	const navtype_t nt_glo[3] = {NAV_GLO_FDMA,NAV_GLO_L1OC,NAV_GLO_L3OC};
	const navtype_t nt_qzs[3] = {NAV_QZS_LNAV,NAV_QZS_CNAV,NAV_QZS_CNV2};
	const navtype_t nt_cmp[5] = {NAV_CMP_D1,NAV_CMP_D2,NAV_CMP_CNV1,NAV_CMP_CNV2,NAV_CMP_CNV3};
	const navtype_t nt_sbs[1] = {NAV_SBS_L1NV};
	const navtype_t nt_irn[2] = {NAV_IRN_LNAV,NAV_IRN_L1NV};

    switch (sys) {
		case SYS_GPS: navids=(char **)navids_gps; sz_=(int *)sz_t[0]; p=nt_gps; break;
		case SYS_GAL: navids=(char **)navids_gal; sz_=(int *)sz_t[1]; p=nt_gal; break;
		case SYS_GLO: navids=(char **)navids_glo; sz_=(int *)sz_t[2]; p=nt_glo; break;
		case SYS_QZS: navids=(char **)navids_qzs; sz_=(int *)sz_t[3]; p=nt_qzs; break;
		case SYS_CMP: navids=(char **)navids_cmp; sz_=(int *)sz_t[4]; p=nt_cmp; break;
		case SYS_SBS: navids=(char **)navids_sbs; sz_=(int *)sz_t[5]; p=nt_sbs; break;
		case SYS_IRN: navids=(char **)navids_irn; sz_=(int *)sz_t[6]; p=nt_irn; break;
    }
    ...
    navtype=p[j];
    *sz=sz_?sz_[j]:0;
    trace(4,"select_navtype: sys=%2d navtype=%d %s\n",sys,navtype,navids[j]);
    return navtype;
}
...
static int parse_rnxnavb_sto(FILE *fp, int sys, nav_t *nav, char *buff)
{
	...
	detect_utc_src(buff+sp+20,&src,&dst);
				...
				int sys, int *type, eph_t *eph, geph_t *geph, seph_t *seph)
{
    gtime_t toc;
    double data[64];
    int i=0,j,prn,sat=0,sp=3,mask,sz=31,ret;
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            if (ver>=4.0) { /* ver.4 */
                if (buff[0]!='>'){
                    trace(2,"rinex nav format error: %23.23s\n",buff);
                    return 0;                   
                }
                sprintf(id,"%.3s",buff+6);
                sat=satid2no(id);
				sys=satsys(sat,NULL);

                if (strncmp(buff+2,"EPH",3)==0) { /* EPH record */
					navtype=select_navtype(sys,buff+10,&sz); /* reset navid */
					if (navtype==NAV_NONE) return 0;
					fgets(buff,MAXRNXLEN,fp); /* read next line for data fields */
					sprintf(id,"%.3s",buff);
					if (satid2no(id) != sat) {
						trace(2,"rinex nav satellite mismatch: %23.23s\n",buff);
						return 0;
					}
					sp=4;
                }
            ...
            else if (ver>=3.0||sys==SYS_GAL||sys==SYS_QZS) { /* ver.3 or GAL/QZS */
                sprintf(id,"%.3s",buff);
                sat=satid2no(id);
                sp=4;
                if (ver>=3.0) {
			...
			if (sys==SYS_GLO&&i>=sz) {
                ...
                *type=1;
				return decode_geph(ver,sat,toc,data,geph,navtype);
            }
            else if (sys==SYS_SBS&&i>=sz) {
                if (!(mask&sys)) return 0;
                *type=2;
				return decode_seph(ver,sat,toc,data,seph,navtype);
            }
            else if (i>=sz) {
                if (!(mask&sys)) return 0;
                *type=0;
                return decode_eph(ver,sat,toc,data,eph,navtype);
            }
        }
    }
    return -1;
...
static void out_iono(FILE *fp, int sys, const rnxopt_t *opt, const nav_t *nav)
{
	...
	if (sys&opt->navsys&SYS_GPS) {
		if (opt->rnxver<=211) out_iono_sys(fp,"",nav->ion[ION_GPS_LNAV_KLOB].d,8);
		else out_iono_sys(fp,"GPS",nav->ion[ION_GPS_LNAV_KLOB].d,8);
	}
	if ((sys&opt->navsys&SYS_GAL)&&opt->rnxver>=212) {
		out_iono_sys(fp,"GAL",nav->ion[ION_GAL_IFNV_NEQN].d,3);
	}
	if ((sys&opt->navsys&SYS_QZS)&&opt->rnxver>=302) {
		out_iono_sys(fp,"QZS",nav->ion[ION_QZS_LNAV_KLOB].d,8);
	}
	if ((sys&opt->navsys&SYS_CMP)&&opt->rnxver>=302) {
		out_iono_sys(fp,"BDS",nav->ion[ION_CMP_D1D2_KLOB].d,8);
	}
...
/* select navtype id */
static int select_sysid(int sys, navtype_t navtype)
{
	int id=-1;
	...
	switch (sys) {
		...
		case SYS_CMP:
			id=(navtype==NAV_CMP_D1||navtype==NAV_CMP_D2)?1:2;break;
...
static void out_iono_sys4(FILE *fp, const ion_t *ion)
{
	...
	fprintf(fp, "> ION %-3s %4s %4s\n",buff,ion_navtype[id],reg);
	time2epoch(ion->ttm,ep);
	fprintf(fp, "    %4.0f %2.0f %2.0f %2.0f %2.0f %2.0f",
		ep[0],ep[1],ep[2],ep[3],ep[4],ep[5]);

	outnavf(fp,ion->d[0]); /* alp0 */
	outnavf(fp,ion->d[1]); /* alp1 */
	outnavf(fp,ion->d[2]); /* alp2 */
	fprintf(fp,"\n");
	fprintf(fp,"    ");
	outnavf(fp,ion->d[3]); /* alp3 */

	if (sys!=SYS_GAL) {
     	outnavf(fp,ion->d[4]); /* bet0 */
		outnavf(fp,ion->d[5]); /* bet1 */
		outnavf(fp,ion->d[6]); /* bet2 */
		fprintf(fp,"\n");
		fprintf(fp,"    ");
		outnavf(fp,ion->d[7]); /* bet3 */
	}
	fprintf(fp,"\n");
}
/* output EOP for a system (RINEX v4)-------------------------------*/
static void out_eop_sys4(FILE *fp, const eop_t *eop)
{
	double ep[6],ttm;
    char buff[4];

	if (norm(eop->xp,3)<=0.0&&norm(eop->yp,3)<=0.0) return;

	if (eop->type<0) return;

    ttm=time2gpst(eop->ttm,NULL);

	satno2id(eop->sat,buff);
	fprintf(fp, "> EOP %-3s %4s\n",buff,eop_navtype[eop->type]);
	time2epoch(eop->ttm,ep);
	fprintf(fp, "    %4.0f %2.0f %2.0f %2.0f %2.0f %2.0f",
		ep[0],ep[1],ep[2],ep[3],ep[4],ep[5]);

	outnavf(fp,eop->xp[0]); /* xp */
	outnavf(fp,eop->xp[1]); /* xpd */
	outnavf(fp,eop->xp[2]); /* xpdd */
	fprintf(fp,"\n");
	...
	fprintf(fp,"    ");
	outnavf(fp,ttm); /* spare */
	outnavf(fp,eop->dut1[0]); /* dut1 */
	outnavf(fp,eop->dut1[1]); /* dut1d */
	outnavf(fp,eop->dut1[2]); /* dut1dd */
	fprintf(fp,"\n");
}
/* output iono corrections for RINEX 4---------------------------------------*/
static void out_iono4(FILE *fp, int sys, const rnxopt_t *opt, const nav_t *nav)
{
    if (!opt->outiono) return;

	if (sys&opt->navsys&SYS_GPS) {
		out_iono_sys4(fp,&nav->ion[TSYS_GPS]);
	}
	if (sys&opt->navsys&SYS_GAL) {
		out_iono_sys4(fp,&nav->ion[TSYS_GAL]);
	}
	if (sys&opt->navsys&SYS_QZS) {
		out_iono_sys4(fp,&nav->ion[TSYS_QZS]);
	}
	if (sys&opt->navsys&SYS_CMP) {
		out_iono_sys4(fp,&nav->ion[TSYS_CMP]);
	}
    if (sys&opt->navsys&SYS_IRN) {
		out_iono_sys4(fp,&nav->ion[TSYS_IRN]);
    }
}
/* output EOP for RINEX 4---------------------------------------*/
static void out_eop4(FILE *fp, int sys, const rnxopt_t *opt, const nav_t *nav)
{
    /*if (!opt->outeop) return;*/

	if (sys&opt->navsys&SYS_GPS) {
		out_eop_sys4(fp,&nav->eop[TSYS_GPS]);
	}
	if (sys&opt->navsys&SYS_GLO) {
		out_eop_sys4(fp,&nav->eop[TSYS_GLO]);
	}
	if (sys&opt->navsys&SYS_QZS) {
		out_eop_sys4(fp,&nav->eop[TSYS_QZS]);
	}
	if (sys&opt->navsys&SYS_CMP) {
		out_eop_sys4(fp,&nav->eop[TSYS_CMP]);
	}
	if (sys&opt->navsys&SYS_IRN) {
		out_eop_sys4(fp,&nav->eop[TSYS_IRN]);
    }
}
...
static void out_time(FILE *fp, int sys, const rnxopt_t *opt, const nav_t *nav)
{
	...
	if (sys&opt->navsys&SYS_GPS) {
		if (opt->rnxver<=211) out_time_sys(fp,"",&nav->sto[TSYS_GPS]);
		else out_time_sys(fp,"GPUT",&nav->sto[TSYS_GPS]);
	}
	if ((sys&opt->navsys&SYS_GLO)&&opt->rnxver>=212) {
        sto_t utc;
        utc=nav->sto[TSYS_GLO];
		/* RINEX 2.12-3.02: tau_C, 3.03- : -tau_C */
        if (opt->rnxver>302) utc.a[0]=-utc.a[0];
		out_time_sys(fp,"GLUT",&utc);
	}
	if ((sys&opt->navsys&SYS_SBS)&&opt->rnxver>=212) {
		out_time_sys(fp,"SBUT",&nav->sto[TSYS_SBS]);
	}
	if ((sys&opt->navsys&SYS_GAL)&&opt->rnxver>=212) {
		out_time_sys(fp,"GAUT",&nav->sto[TSYS_GAL]);
	}
	if ((sys&opt->navsys&SYS_QZS)&&opt->rnxver>=302) {
		out_time_sys(fp,"QZUT",&nav->sto[TSYS_QZS]);
	}
	if ((sys&opt->navsys&SYS_CMP)&&opt->rnxver>=302) {
		out_time_sys(fp,"BDUT",&nav->sto[TSYS_CMP]);
	}
	if ((sys&opt->navsys&SYS_IRN)&&opt->rnxver>=303) {
		out_time_sys(fp,"IRUT",&nav->sto[TSYS_IRN]);
	}
}
...
static int out_time4(FILE *fp, int sys, const rnxopt_t *opt, const nav_t *nav)
{
	int ret=0;

	if (sys&opt->navsys&SYS_GPS) {
		out_time_sys4(fp,"GPUT",&nav->sto[TSYS_GPS]);
	}
	if (sys&opt->navsys&SYS_GLO) {
		out_time_sys4(fp,"GLUT",&nav->sto[TSYS_GLO]);
	}
	if (sys&opt->navsys&SYS_SBS) {
		out_time_sys4(fp,"SBUT",&nav->sto[TSYS_UTC]);
	}
	if (sys&opt->navsys&SYS_GAL) {
		out_time_sys4(fp,"GAUT",&nav->sto[TSYS_GAL]);
	}
	if (sys&opt->navsys&SYS_QZS) {
		out_time_sys4(fp,"QZUT",&nav->sto[TSYS_QZS]);
	}
	if (sys&opt->navsys&SYS_CMP) {
		out_time_sys4(fp,"BDUT",&nav->sto[TSYS_CMP]);
	}
	if (sys&opt->navsys&SYS_IRN) {
		out_time_sys4(fp,"IRUT",&nav->sto[TSYS_IRN]);
	}
	return ret;
}
...
static void out_leaps(FILE *fp, int sys, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    double dt_ls=0.0;
    const sto_t *utc=NULL;
    int wn_lsf=0,dn=0;

    if (!opt->outleaps) return;

    switch (sys) {
        case SYS_GAL: utc=&nav->sto[TUTC_GAL]; break; 
        case SYS_QZS: utc=&nav->sto[TUTC_QZS]; break;
        case SYS_CMP: utc=&nav->sto[TUTC_CMP]; break;
        case SYS_IRN: utc=&nav->sto[TUTC_IRN]; break;
        default     : utc=&nav->sto[TUTC_GPS]; break;
    }

    dt_ls=utc->dt_ls;
    if (dt_ls==0.0) return;
    dn=time2gpst(utc->tlsf,&wn_lsf)/86400;
...
extern int outrnxnavh(FILE *fp, const rnxopt_t *opt, const nav_t *nav)
{
    ...
    if (opt->rnxver<400)  {
		...
		out_leaps(fp,SYS_GPS,opt,nav);
		fprintf(fp,"%60s%-20s\n","","END OF HEADER");
		out_iono4(fp,opt->sep_nav?SYS_GPS:SYS_ALL,opt,nav);
		out_eop4(fp,opt->sep_nav?SYS_GPS:SYS_ALL,opt,nav);
		ret=out_time4(fp,opt->sep_nav?SYS_GPS:SYS_ALL,opt,nav);
		return ret!=EOF;
	}
}

static int detect_navid(int sys, navtype_t navtype, char ***navids)
{
	int i,navid=-1;
	navtype_t *p;

	switch (sys) {
		case SYS_GPS: *navids=(char **)navids_gps;p=(navtype_t *)navtype_gps;break;
		case SYS_GAL: *navids=(char **)navids_gal;p=(navtype_t *)navtype_gal;break;
		case SYS_GLO: *navids=(char **)navids_glo;p=(navtype_t *)navtype_glo;break;
		case SYS_QZS: *navids=(char **)navids_qzs;p=(navtype_t *)navtype_qzs;break;
		case SYS_CMP: *navids=(char **)navids_cmp;p=(navtype_t *)navtype_cmp;break;
		case SYS_SBS: *navids=(char **)navids_sbs;p=(navtype_t *)navtype_sbs;break;
		case SYS_IRN: *navids=(char **)navids_irn;p=(navtype_t *)navtype_irn;break;
	}
...
extern int outrnxnavb(FILE *fp, const rnxopt_t *opt, const eph_t *eph)
{
    ...
    if (opt->rnxver>=400) {
		...
		navid=detect_navid(sys, eph->navtype, &navids);
    ...
    else if ((opt->rnxver>=300&&sys==SYS_GPS)||(opt->rnxver>=212&&sys==SYS_GAL)||
    ...
    /* Line 1 */
    outnavf(fp,v1          ); /* GPS/QZS: IODE, GAL: IODnav, BDS: AODE */
    outnavf(fp,eph->crs    );
    outnavf(fp,eph->deln   );
    outnavf(fp,eph->M0     );
    fprintf(fp,"\n%s",sep  );
	...
	if (((sys==SYS_GPS||sys==SYS_QZS)&&
		...
		outnavf(fp,eph->urai[0]); /* URAI NED0 */
		outnavf(fp,eph->urai[1]); /* URAI NED1 */
   ...
   /* Line 6 */
	if (((sys==SYS_GPS||sys==SYS_QZS)&&(navtype==NAV_GPS_CNAV||navtype==NAV_QZS_CNAV||
		navtype==NAV_GPS_CNV2||navtype==NAV_QZS_CNV2))) {
		outnavf(fp,eph->urai[3]); /* URAI ED */
		outnavf(fp,eph->svh    );
		outnavf(fp,eph->tgd[0] ); /* GPS/QZS:TGD, GAL:BGD E5a/E1, BDS: TGD1 B1/B3 */
		outnavf(fp,eph->urai[2]); /* URAI NED2 */
	} else if (((sys==SYS_CMP)&&(navtype==NAV_CMP_CNV1||navtype==NAV_CMP_CNV2||
		navtype==NAV_CMP_CNV3))) {
        outnavf(fp,eph->urai[0]); /* SISAI_oe */
        outnavf(fp,eph->urai[1]); /* SISAI_ocb */
        outnavf(fp,eph->urai[2]); /* SISAI_oc1 */
        outnavf(fp,eph->urai[3]); /* SISAI_oc2 */
	} else if (((sys==SYS_IRN)&&(navtype==NAV_IRN_L1NV))) {
        outnavf(fp,uravalue(eph->sva));
        outnavf(fp,eph->svh    );
        outnavf(fp,eph->tgd[0] ); /* tgd L1P-L5 */
        outnavf(fp,eph->tgd[1] ); /* tgd S-L5 */        
    } else {
        if (sys==SYS_GAL) {
	...
	if (((sys==SYS_GPS||sys==SYS_QZS)&&
        ...
        if (navtype==NAV_CMP_CNV1||navtype==NAV_CMP_CNV2) {
            ...
            outnavf(fp,eph->tgd[4]); /* BDS: TGD B2bI */
        }
        fprintf(fp,"\n%s",sep  );
        ...
        outnavf(fp,eph->tgd[3]); /* ISC L1D(L1O) */
        outnavf(fp,eph->tgd[4]); /* ISC L1P(S) */
        outnavf(fp,eph->tgd[5]); /* ISC L1D(S) */
        fprintf(fp,"\n%s",sep  ); 
...
extern int outrnxgnavb(FILE *fp, const rnxopt_t *opt, const geph_t *geph)
{
    ...
    if (opt->rnxver>=400) { /* ver.4 */
        ...
        navid=detect_navid(sys, geph->navtype, &navids);
        fprintf(fp,"> %-3s %-3s %-4s\n","EPH", code, navids[navid]);
        fprintf(fp,"%-3s %04.0f %02.0f %02.0f %02.0f %02.0f %02.0f",code,ep[0],
                ep[1],ep[2],ep[3],ep[4],ep[5]);
        sep="    ";   
    } else if (opt->rnxver<=299) { /* ver.2 */
        fprintf(fp,"%2d %02d %02.0f %02.0f %02.0f %02.0f %04.1f",prn,
                (int)ep[0]%100,ep[1],ep[2],ep[3],ep[4],ep[5]);
        sep="   ";
    }
...
extern int outrnxhnavb(FILE *fp, const rnxopt_t *opt, const seph_t *seph)
{
    ...
    if (opt->rnxver>=400) { /* ver.4 */
        ...
        navid=detect_navid(sys, NAV_SBS_L1NV, &navids);
		fprintf(fp,"> %-3s %-3s %-4s\n","EPH", code, navids[navid]);
        fprintf(fp,"%-3s %04.0f %02.0f %02.0f %02.0f %02.0f %02.0f",code,ep[0],
                ep[1],ep[2],ep[3],ep[4],ep[5]);
        sep="    ";   
