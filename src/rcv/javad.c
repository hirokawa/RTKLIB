...
static int decode_SI(raw_t *raw)
{
    ...
    for (i=0;i<raw->obuf.n&&i<MAXOBS;i++) {
		...
		else if (usi<= 69) sat=satno(SYS_GLO,usi-37);  /*  38- 70: GLONASS */
...
/* decode [ESI] extended satellite indices -----------------------------------*/
static int decode_SX(raw_t *raw)
{
	int i,ssid,svid,sys,sat;
	char *msg;
	uint8_t *p=raw->buff+5;

	if (!checksum(raw->buff,raw->len)) {
		trace(2,"javad ESI checksum error: len=%d\n",raw->len);
		return -1;
	}
	raw->obuf.n=(raw->len-6)/2;
	...
	for (i=0;i<raw->obuf.n&&i<MAXOBS;i++) {
		...
		switch (ssid) {
			case 1:sys=SYS_GPS;break;     /*   1- 254: GPS */
			case 2:sys=SYS_GLO;break;     /*  FCN -7..24,25,127: GLONASS */
			case 3:sys=SYS_SBS;break;     /* 120-254: SBAS */
			case 4:sys=SYS_GAL;break;     /*  1-254: GALILEO */
			case 5:sys=SYS_QZS;break;     /* 193-254: QZSS */
			case 6:sys=SYS_CMP;break;     /* 1-254: BeiDou */
			case 7:sys=SYS_IRN;break;     /* 1-254: NavIC */
			case 9:sys=SYS_GLC;break;     /* 1-254: GLONASS CDMA */
			default: continue;
		}
        ...
        /* glonass fcn (frequency channel number) */
		if (sys==SYS_GLO) raw->freqn[i]=(svid>127)?svid-256:svid;
    }
    trace(4,"decode_ESI: nsat=raw->obuf.n\n");
...
/* decode raw L2C/L5 CNAV data ---------------------------------------------------*/
static int decode_cnav(uint8_t *buff, int len, int sat, raw_t *raw)
{
    uint8_t msg[1024]={0};
	int i,j,preamb,prn,msgid,tow,alert,ofst,sys=satsys(sat,NULL),id;
	eph_t eph={0};
	double ion[4]={0},utc[8]={0},eop[7]={0};
    
    trace(3,"decode_cnav len=%2d sat=%2d L2C/L5 CNAV\n",len,sat);
    ...
    if (preamb!=PREAMB_CNAV) {
		trace(2,"javad *d sat=%2d CNAV preamble error preamb=%02X\n",preamb);
        return -1;
    }
    trace(3,"CNAV: sat=%2d prn=%2d msgid=%2d tow=%6d alert=%d\n",sat,prn,
          msgid,tow,alert);
	...
	memcpy(raw->subfrm[sat-1]+SF_OFST_GPS_CNAV+ofst*35,msg,35);
...
{
	int i,sys=satsys(sat,NULL),id;
	uint8_t *p=raw->buff+5;
	eph_t eph={0};
	double ion[4]={0},utc[8]={0},eop[7]={0};

	trace(3,"javad *d len=%2d sat=%2d L1C CNAV2\n",len,sat);

    /* copy data */
	for (i=0,p+=6;i<57;i++,p+=4) {
		setbitu(buff,32*i,32,U4(p));
	}
	if (!decode_gps_cnav2(buff,&eph,NULL,NULL,NULL,NULL,sys,1)) return 0;
...
static int decode_nd(raw_t *raw, int sys)
{
    ...
    switch (type) {
		...
		case 1: return decode_cnav  (p,len,sat,raw); /* L2C CNAV */
        case 2: return decode_cnav  (p,len,sat,raw); /* L5  CNAV */
...
static int decode_ED(raw_t *raw)
{
    ...
    if (!checksum(raw->buff,raw->len)) {
        trace(2,"javad ED checksum error: len=%d\n",raw->len);
        return -1;
    }

    prn =U1(p); p+=1;
    time=U4(p); p+=4;
    type=U1(p)&0x7; p+=1; /* 0:I/NAV(E1B),1:F/NAV,2:I/NAV(E5B),6:C/NAV(E6B) */
    len =U1(p); p+=1;

    trace(3,"decode_ED: prn=%3d type=%1d\n",prn, type);

    if (raw->len!=14+len) {
        trace(2,"javad ED length error: len=%d\n",raw->len);
        return -1;
    }
    if (raw->outtype) {
        msg=raw->msgtype+strlen(raw->msgtype);
    ...
    if (!(sat=satno(SYS_GAL,prn))) {
        trace(2,"javad ED satellite error: prn=%d\n",prn);
        return 0;
    }
    trace(4,"sat=%2d time=%7d type=%d len=%3d\n",sat,time,type,len);
...
static int decode_cd(raw_t *raw)
{
    ...
    prn =U1(p); p+=1;
    tow=U4(p); p+=4;	/* Beidou time of receiving message */
    type=U1(p); p+=1;
    otype = (type>>7)&1; /* 1:GEO */
    b2bq = (type>>6)&1;  /* 1: B2b data decoded from B2bq */
    signal = type&0x3f;
    /*
     * type of data:
     *  b0-5: signal type: 0:B1,1:B2,2:B3,3:B1C,5:B2a,6:B2b
     *  b6: 1: B2b data was decoded from B2bq (D2)
     *  b7: 1: D2 structure from GEO
     */
    len =U1(p); p+=1;

    trace(3,"decode_cd: len=%3d prn=%3d time=%d type=%x\n",len,prn,tow,type);

    if (raw->len!=14+len*4) {
        trace(2,"javad cd length error: len=%d\n",raw->len);
        return -1;
    }
    if (raw->outtype) {
        msg=raw->msgtype+strlen(raw->msgtype);
        sprintf(msg," prn=%3d tow-bdt=%7d type=%d B2bq:%d D2-GEO:%d",
                prn,tow,signal,b2bq,otype);
    }
    if (!(raw->ephsat=sat=satno(SYS_CMP,prn))) {
        trace(2,"javad cd satellite error: prn=%d\n",prn);
        return 0;
    }
    ...
    switch (signal) {
        ...
        case 1: /* B2I D1/D2 */
        case 2: /* B3I D1/D2 */
            id = getbitu(subfrm,15,3);
            if (otype==0) { /* MEO/IGSO */
                if (id==0||id>5) return 0;
                memcpy(raw->subfrm[sat-1]+(id-1)*38,subfrm,38);
				if(!decode_bds_d1(raw->subfrm[sat-1],&eph,NULL,NULL)) return 0;
			} else { /* GEO */
				if (id==1) { /* ephemeris */
					page = getbitu(subfrm,42,4);
					if (page==0||page>10) return 0;
					memcpy(raw->subfrm[sat-1]+(page-1)*38,subfrm,38);
				} else if (id==5) { /* almanac, iono, utc */
					page = getbitu(subfrm,43,7);
					if (page!=102) return 0;
					memcpy(raw->subfrm[sat-1]+10*38,subfrm,38);
				} else { /* SF2/3/4 diff-correction, integrity */
			...
			if (otype==0) { /* MEO/IGSO */
				if(decode_bds_d1(raw->subfrm[sat-1],NULL,ion,NULL)) {
					set_ion_param(raw,sat,NAV_CMP_D1,ion);
				}
				if(decode_bds_d1(raw->subfrm[sat-1],NULL,NULL,utc)) {
        ...
        case 3: /* B1C B-CNAV1 */
            eph.sat=sat;
        	if(!decode_bds_cnav1(subfrm,&eph,NULL,NULL,NULL,1)) return 0;
            if (!strstr(raw->opt,"-EPHALL")) {
                if (eph.iode==raw->nav.eph[sat-1+MAXSAT].iode&&
					timediff(eph.toe,raw->nav.eph[sat-1+MAXSAT].toe)==0.0&&
                    timediff(eph.toc,raw->nav.eph[sat-1+MAXSAT].toc)==0.0) return 0;
			}
			if(decode_bds_cnav1(subfrm,NULL,ion,NULL,NULL,1)) {
				set_ion_param(raw,sat,NAV_CMP_CNV1,ion);
			}
			if(decode_bds_cnav1(subfrm,NULL,NULL,utc,NULL,1)) {
				set_utc_param(raw,sat,NAV_CMP_CNV1,utc);
			}
			if(decode_bds_cnav1(subfrm,NULL,NULL,NULL,eop,1)) {
			...
			return 2;
		case 5: /* B2a B-CNAV2 */
			id = getbitu(subfrm,6,6);
			if (id==10||id==11) {
				memcpy(raw->subfrm[sat-1]+(id-10)*36+SF_OFST_BDS_CNAV2,subfrm,len*4);
			} else if (id>=30&&id<40) {
				memcpy(raw->subfrm[sat-1]+36*2+SF_OFST_BDS_CNAV2,subfrm,len*4);
			} else if (id==40) {
				memcpy(raw->subfrm[sat-1]+36*3+SF_OFST_BDS_CNAV2,subfrm,len*4);
			}
		...
		case 6: /* B2b */
			if (prn>=59) { /* GEO PPP */
				...
				if (id==10) {
					memcpy(raw->subfrm[sat-1]+SF_OFST_BDS_CNAV3,subfrm,64);
				} else if (id==30) {
					memcpy(raw->subfrm[sat-1]+SF_OFST_BDS_CNAV3+64,subfrm,64);
				} else {
					return 0;
				}
				if (!decode_bds_cnav3(raw->subfrm[sat-1]+SF_OFST_BDS_CNAV3,&eph,
					NULL,NULL,NULL,1)) return 0;
				if (!strstr(raw->opt,"-EPHALL")) {
					if (timediff(eph.toe,raw->nav.eph[sat-1+MAXSAT*3].toe)==0.0&&
						timediff(eph.toc,raw->nav.eph[sat-1+MAXSAT*3].toc)==0.0) return 0;
				}
				if (decode_bds_cnav3(raw->subfrm[sat-1]+SF_OFST_BDS_CNAV3,NULL,
					ion,NULL,NULL,1)) {
...
static int decode_id(raw_t *raw)
{
    ...
    prn=U1(p); p+=1;
    time=U4(p); p+=4;
    type=U1(p); p+=1; /* 0:L5,1:S,3:L1 */
    len=U1(p); p+=1;

    if (raw->len!=len*4+14) {
        trace(2,"javad id length error: len=%d\n",raw->len);
        return -1;
    }

    if (raw->outtype) {
        msg=raw->msgtype+strlen(raw->msgtype);
        sprintf(msg," prn=%3d time=%6d type=%d",prn,time,type);
    }

    if (!(sat=satno(SYS_IRN,prn))) {
        trace(2,"javad id satellite error: prn=%d\n",prn);
        return 0;
    }
	...
	if (type==0) { /* L5 */
		...
		trace(3,"decode_id: prn=%2d time=%7d type=%d tow=%6d id=%2d\n",
			prn,time,type,tow,id);
...
static int decode_javad(raw_t *raw)
{
    ...
    if (!strncmp(p,"SX",2)) return decode_SX(raw); /* extended satellite indices */
